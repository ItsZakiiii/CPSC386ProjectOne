...
class AlienFleet:
    ...
    alien_images = [pg.image.load(f'images/Crab1{n}.png') for n in range(3)]
    alien_images1 = [pg.image.load(f'images/Crab1{n}.png') for n in range(3)]
    alien_images2 = [pg.image.load(f'images/Crab2{n}.png') for n in range(3)]
    alien_images3 = [pg.image.load(f'images/Crab3{n}.png') for n in range(3)]
    alien_images4 = [pg.image.load(f'images/Crab4{n}.png') for n in range(3)]
    ...
    def create_alien(self, row, col):
        ...
        if row%4 == 0:
            alien = Alien(game=self.game, ul=(x, y), v=self.v, image_list=AlienFleet.alien_images3)
            self.fleet.add(alien)
        elif row == 1:
            alien = Alien(game=self.game, ul=(x, y), v=self.v, image_list=AlienFleet.alien_images2)
            self.fleet.add(alien)
        else:
            alien = Alien(game=self.game, ul=(x, y), v=self.v, image_list=AlienFleet.alien_images1)
            self.fleet.add(alien)
...
class Alien(Sprite):
    def __init__(self, game, image_list, start_index=0, ul=(0, 100), v=Vector(1, 0)):
        ...
        self.settings = game.settings
        self.image = pg.image.load('images/Crab10.png')
        self.screen_rect = self.screen.get_rect()
