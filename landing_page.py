...
class LandingPage:
    alien_one_imgs =  [pg.image.load(f'images/Crab1{n}.png') for n in range(2)]
    alien_two_imgs = [pg.image.load(f'images/Crab2{n}.png') for n in range(2)]
    alien_three_imgs = [pg.image.load(f'images/Crab3{n}.png') for n in range(2)]
    ufo_imgs = [pg.image.load(f'images/Crab4{n}.png') for n in range(2)]

    def __init__(self, game):
        self.screen = game.screen
        self.landing_page_finished = False
        self.highscore = game.stats.get_highscore()
        self.bg_img = pg.transform.scale(pg.image.load('images/background.png'),
                                         (0, 0))
        ...
        strings = [('SPACE', WHITE, headingFont), ('INVADERS', GREEN, subheadingFont),
                ('= 10 PTS', GREY, font), ('= 10 PTS', GREY, font),
                            ('= 10 PTS', GREY, font), ('= 100', GREY, font),
               # ('PLAY GAME', GREEN, font), 
                ('HIGH SCORES' + str(self.highscore), GREY, font)]
