...
class SbElements:
    ...
    def update(self):
        ...
        else: self.score_rect.left = self.ul[1]
...
class ScoreBoard:
    def __init__(self,game):
        ...
        self.score= SbElements(screen = screen, bg_color=game.bg_color, ul = (sr.right-40,20),font = font,get_score = self.stats.get_score)

        self.highscore= SbElements(screen=screen, bg_color=game.bg_color,ul=(sr.centerx,20),font=font, get_score=self.stats.get_highscore)

        self.level = SbElements(screen=screen, bg_color=game.bg_color,ul=(sr.right-40,50),
                                  font=font,get_score=self.stats.get_level, round = False)
        self.ships = Group()
        self.update()
